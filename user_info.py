...
def generate_user_parameters(number_of_users=1000, mu1=4, sigma1=2, mu2=5,
    ...
    # Generate user_id's that are non-sequential and give them an average
    # number of interactions
    # for each version
    user_parameters['UID'] = (np.random.random(
        size=number_of_users)*10*number_of_users).astype(int)
    user_parameters['avg_orig'] = np.clip(np.random.normal(
        loc=mu1, scale=sigma1, size=number_of_users), 0.1, np.inf)
    user_parameters['avg_test'] = np.clip(np.random.normal(
        loc=mu2, scale=sigma2, size=number_of_users), 0.1, np.inf)
...
def create_clean_user_data(user_parameters, total_logins=50000):
    ...
    user_logins = np.random.poisson(
        lam=total_logins / number_of_users, size=number_of_users)

    # Create random login times in 2020 and 2021 for each user
    login_times = np.hstack([1584803805
                            + (np.random.random(size=this_login_num)
                                * 86400*365).astype(int)
                            for this_login_num in user_logins])
    user_ids = np.hstack([[this_user, ]*this_login_num for this_user,
                          this_login_num in zip(unique_users, user_logins)])
    ...
    for this_userdata, new_data, thiskey in zip([orig_userdata, test_userdata],
                                                [new_orig_data, new_test_data],
                                                ['orig', 'test']):

        for _, this_login in tqdm(this_userdata.iterrows(),
                                  total=len(this_userdata)):
            ...
            #                                'msg':msgs,
            #                                'UID':[this_login.UID,]*(num_interactions+1)})
    ...
    orig_userdata = orig_userdata.append(
        pd.DataFrame.from_dict(new_orig_data), ignore_index=True)
    test_userdata = test_userdata.append(
        pd.DataFrame.from_dict(new_test_data), ignore_index=True)

    orig_userdata['version'] = 'orig'
    test_userdata['version'] = 'test'

    return orig_userdata.append(test_userdata, ignore_index=True
...
def add_noise(userdata, noise_entries=25000):
    ...
    random_times = np.random.random(
        size=noise_entries) * (max_time - min_time) + min_time
    random_times = random_times.astype('datetime64[s]')

    random_users = np.random.choice(
        np.unique(userdata.UID), size=noise_entries, replace=True)

    random_msgs = [f'Signal_Level_{int(percent):02}'
                   for percent in np.random.random(size=noise_entries)*100.]

    orig_version = (['orig', ]
                    * sum(random_times <= userdata.timestamp[
                        userdata.version == 'orig'].iloc[-1].to_datetime64()))
    test_version = (['test', ]
                    * sum(random_times > userdata.timestamp[
                        userdata.version == 'orig'].iloc[-1].to_datetime64()))
    ...
    userdata = userdata.append(pd.DataFrame.from_dict(
        {'timestamp': random_times,
         'UID': random_users,
         'msg': random_msgs,
         'version': random_version}), ignore_index=True)
    userdata = userdata.sort_values('timestamp').reset_index(drop=True)
